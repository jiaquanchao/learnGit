  ...
  <head>
	...
	function backBatch(){
		if($('.ids:checked').size()==0){
			...
			if(confirm("是否确定还原选中的信息"))
		$('#deleteform').attr('action', '${pageContext.request.contextPath }/people/action_backBatch.both');
		$('#deleteform').submit();
	}
	    ...
	    <s:iterator value="#page.queryResult" var="people">
		    	<tr onmouseover="this.style.background = '#aaa'" onmouseout="this.style.background = 'white'">
		    		<td><input type="checkbox" class="ids" name="ids" value="${people.id}"/></td>
				    	...
				    	<td>${city.cityName}</td>	
			    		<td><a href="${pageContext.request.contextPath }/people/action_back.both?id=${people.id }" onClick="javascript:return confirm('是否确定还原')">还原</a></td>
		    	</tr> 
	    </s:iterator>
	    <tr>
	    	<td><h5><label for="check"><input type="checkbox" id="check" class="ids" onclick="$('.ids').attr('checked',this.checked);"/>全选</label></h5></td>
	    	<td colspan="6"><input type="button" onclick="backBatch();" value="还原"/>&nbsp;<input type="button" onclick="deleteBatch();" value="彻底删除"/></td>
	    </tr>
	    </form>
	</table>
  </body>
